  ...
  <div class="d-flex justify-content-between align-items-center my-2 mx-0">
    ...
    <form id="formWorkFlow" autocomplete="off">
      ...
      <div class=" ">
        ...
        <a href="{{ route('dashboard') }}" class="btn btn-light mx-1 me-auto">Kembali</a>
      </div>
  </div>
    ...
    <div class="col-lg-8 form">
      <div class="row p-1 ">
        <div class="col form-group">
          ...
          <input type="text" class="form-control" name="modul" value="{{$data->modul}}" id="modul">
      ...
      <div class="row">
        <div class="col form-group">
          ...
          <select class="form-control" name="type" id="type" style="width:100%">
            <option value="">Pilih Type Modul</option>
            <option value="Custom" {{ $data->type == 'Custom' ? 'selected' : '' }}>Custom</option>
            <option value="HRIS" {{ $data->type == 'HRIS' ? 'selected' : '' }}>HRIS</option>
            <option value="Total Amount >=" {{ $data->type == 'Total Amount >=' ? 'selected' : '' }}>Total Amount >=</option>
            <option value="Total Amount >" {{ $data->type == 'Total Amount >' ? 'selected' : '' }}>Total Amount ></option>
            <option value="Total Amount <" {{ $data->type == 'Total Amount <' ? 'selected' : '' }}>Total Amount <</option> </select> </div> <div class="col form-group">
                <label for="value" class="form-label">Value/Level</label>
                <input type="text" class="form-control" disabled id="value" value="{{$data->value}}" name="value">
        </div>
      </div>
